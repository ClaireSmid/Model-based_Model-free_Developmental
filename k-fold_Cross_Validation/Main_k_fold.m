...
%     testdata = adltgroupdata.subdata(i);        % get participant data
    ...
    for p = 1:length(fold)
        ...
        train(i).fold(p) = struct('id',testdata.id(1),'fold',fold(p,1:3),'N',length(trial),'block',block,'rews',rews,'stimuli',stimuli,...
        ...
        if unique(choice) == -1
            skip_test = 1;
            % if they have less than 10 valid (non-missed) trials in the
            % test block, skip this test fold.
        elseif (length(choice) - sum(choice == -1)) < 10
            skip_test = 1;
        else
            skip_test = 0;      
        end
        ...
        test(i).fold(p) = struct('id',testdata.id(1),'fold',fold(p,4),'N',length(trial),'block',block,'rews',rews,'stimuli',stimuli,...
            'stake',stake,'points',points,'s',s,'choice',choice,'trial',trial,'timeout',timeout,'rt',rt); 
    ...
    %     101d.params = fit(model(101d.train))
        ...
        nstarts = 25;               % set random starting locations of optimizer to 25
    ...
    %     101#.model_accuracy = mean(p#)
        ...
        test(i).modelfitP6(p) = nanmean(choiceProbs);
        ...
        % save the mean choice probability
        test(i).modelfitP7(p) = nanmean(choiceProbs);
    ...
    test(i).mean_accuracyP6 = mean([test(i).modelfitP6(1) test(i).modelfitP6(2) ...
                test(i).modelfitP6(3) test(i).modelfitP6(4)],'omitnan');
            
    test(i).mean_accuracyP7 = mean([test(i).modelfitP7(1) test(i).modelfitP7(2) ...
                test(i).modelfitP7(3) test(i).modelfitP7(4)],'omitnan');
                  
    test(i).mean_SD_accuracyP6 = mean([test(i).modelfitP6_SD(1) test(i).modelfitP6_SD(2) ...
                test(i).modelfitP6_SD(3) test(i).modelfitP6_SD(4)],'omitnan');
            
    test(i).mean_SD_accuracyP7 = mean([test(i).modelfitP7_SD(1) test(i).modelfitP7_SD(2) ...
        test(i).modelfitP7_SD(3) test(i).modelfitP7_SD(4)],'omitnan');
    ...
    % just checking if the accuracy is over 50
...
% save results
save k_fold_CV_test_kids_2Oct20.mat test
save k_fold_CV_train_kids_2Oct20.mat train