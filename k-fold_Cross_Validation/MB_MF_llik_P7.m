function  [LL,choiceProb] = MB_MF_llik_P7(x,data)
...
%% fout hier
choiceProb = nan(length(data.choice),1);
% fprintf('\nchoiceProb this trial: %.2f');

% loop through trials
for t = 1:data.N
    
    if t == 1 || data.block(t) ~= data.block(t-1)
        Qmf = ones(2,2)*0.5;
        Q2 = ones(2,1)*0.5;     % Q(s,a): state-action value function for Q-learning
        M = [0 0; 0 0];         % last choice structure
        R = [0; 0];             % last choice structure
    ...
    if data.rt(t,1) == -1 || data.rt(t,2) == -1
        continue
    end
    ...
    choiceProb(t) = exp(b*Q(a))/sum(exp(b*Q));
%     disp(choiceProb(t));
%     fprintf('\n');
