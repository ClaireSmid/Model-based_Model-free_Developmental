function results = mfit_optimize(likfun,param,data,nstarts)
    ...
    if isfield(data,'id')
        results.id = data.id;           % added in by Claire 20/01/20
    end
    results.K = K;
    results.nstarts = nstarts;      % added in by Claire 16/09/20
    results.trials = length(data);  % added in by Claire 16/09/20
    ...
    for s = 1:length(data)
        ...
        %disp(['Subject ',num2str(s)]); % turned off by Claire 20/01/20
