%% Parameter recovery 7-parameter model
% parameter recovery for the 7-parameter model (2 ws) 
...
% x4 w_lo mixing weight
wbounds = linspace(0,1,num_agents);
w_lo = (unifrnd(wbounds(1),wbounds(num_agents),num_agents,1));

% x5 w_hi mixing weight
wbounds = linspace(0,1,num_agents);
w_hi = (unifrnd(wbounds(1),wbounds(num_agents),num_agents,1));
...
for i = 1:num_agents % for every simulation
    ...
    data = MBMF_generate_sim_7Params(i,x,rews);
    
    % 2.4 save the parameters used for later 
    data.P7_init_it = it(i);
    data.P7_init_lr = lr(i);
    data.P7_init_et = et(i);
    data.P7_init_w_lo = w_lo(i);
    data.P7_init_w_hi = w_hi(i);
    data.P7_init_st = st(i);
    data.P7_init_repst = repst(i);
    
    % 2.5 save data
    Sdata = [Sdata; data];
end

save simulated_data_P7.mat Sdata
clear Sdata

%% Step 2. Start simulations
nstarts = 100;   % to avoid local optima % increased this to 25

load simulated_data_P7.mat
...
parfor z = 1:num_agents  
        ...
        params = set_simulation_params7;
        f = @(x,data) MB_MF_llik_simulation_7Params(x,data);
        ...
        data.P7_res_lr = a(2);
        data.P7_res_et = a(3);
        data.P7_res_w_lo = a(4);
        data.P7_res_w_hi = a(5);
        data.P7_res_st = a(6);
        data.P7_res_repst = a(7);
        
        Pdata = [Pdata, data]; 

end 

save parameter_recovery_P7.mat Pdata
