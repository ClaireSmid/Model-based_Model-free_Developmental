...
if part == 1 % 7 screens
    switch screen
        ...
        case 4
            ...
            rect1 = [pms.origin(1)-1.25*aliensize pms.origin(2)-1.3*aliensize ...
                pms.origin(1)-0.25*aliensize pms.origin(2)-0.3*aliensize];
            rect2 = [pms.origin(1)+0.25*aliensize pms.origin(2)-1.3*aliensize ...
                pms.origin(1)+1.25*aliensize pms.origin(2)-0.3*aliensize];
            
            cavesize = 200; % draw two caves
            cave1 = [pms.origin(1)-1.5*cavesize pms.origin(2)-0.05*cavesize ... % added
                pms.origin(1)-0.5*cavesize pms.origin(2)+0.95*cavesize];
            cave2 = [pms.origin(1)+0.5*cavesize pms.origin(2)-0.05*cavesize ... % added
                pms.origin(1)+1.5*cavesize pms.origin(2)+0.95*cavesize];
        ...
        case 5
            ...
            itemsize = 75; % show blue treasure (x1)
        ...
        case 6
            message = 'Aliens move around in the caves on their planet. If they are in a good part of their cave, they can give you lots of treasure!\n\n\n\n\n\n\nBut if they are on a bad part of their cave, they won''t have much treasure to give you:';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
            
            treasuresize = [500 216]*0.75; 
            rect1 = [pms.origin(1)-0.5*treasuresize(1) pms.origin(2)+0.75*treasuresize(2) ...
                pms.origin(1)+0.5*treasuresize(1) pms.origin(2)+1.75*treasuresize(2)];
...
%             antimattersize = [500 216]*0.75; 
            rect2 = [pms.origin(1)-0.5*treasuresize(1) pms.origin(2)+0.75*treasuresize(2) ...
                pms.origin(1)+0.5*treasuresize(1) pms.origin(2)+1.75*treasuresize(2)];
...
%             Screen('DrawTexture',pms.wid,pms.caveTexture1,[],cave1); % added
        ...
        case 8
            message = 'The space treasure that you collect in this game will count towards your payment at the end.\n\nFor every 100 points, you will get 0.33 GBP. On average, people win about 7.6GBP, and some have won more than 12GBP.\n\n\nMake sure you try and collect as much treasure as you can!';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
...
if part == 9 % 5 screens
    switch screen
        ...
        case 1
            message = 'Well done!\n\nThere''s one last thing you need to know.\n\nYou have a special boostray that can turn the treasure you collect from the aliens into special gold treasure. It is worth FIVE TIMES more than the blue treasure!\n\n\n\n\n\n\nHowever, because your boost ray breaks all the time, you are not sure when you can use it!';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
            
            itemsize = 150;
            rect1 = [pms.origin(1)-0.55*itemsize pms.origin(2)-0.5*itemsize ...
                pms.origin(1)+0.55*itemsize pms.origin(2)+0.5*itemsize];
            Screen('DrawTexture',pms.wid,pms.BoostrayTexture,[],rect1+[0 0.05*pms.wRect(4) 0 0.05*pms.wRect(4)]);


        case 2
            message = 'You will now see if you can use your special boost ray before every spaceship you choose.\n\n\n\n\n\n\n\nYou will either see a working boost ray with sparks, or an off boost ray with a cross.';            
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
            
%             imgsize = [600 220]*0.5;
%             imgsize = [600 220];
%             rect = [pms.origin(1)-1*imgsize(1) pms.origin(2)-0.5*imgsize(2) ...
%                 pms.origin(1)+1*imgsize(1) pms.origin(2)+0.5*imgsize(2)];
%             Screen('DrawTexture',pms.wid,pms.exampleStakesTexture,[],rect+[0 -0.12*pms.wRect(4) 0 -0.12*pms.wRect(4)]);
            
            Screen('DrawTexture', pms.wid, pms.exampleStakesTexture);
            

        case 3
            message = 'If you see the boost ray with the sparks on the screen, this means that this time you will turn the blue treasure into gold treasure that is worth FIVE TIMES more!\n\n\n\n\n\n\n';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
...
%             
            ...
            rect1 = [pms.origin(1)-0.55*itemsize pms.origin(2)-0.5*itemsize ...
                pms.origin(1)+0.55*itemsize pms.origin(2)+0.5*itemsize];
            Screen('DrawTexture',pms.wid,pms.BoostrayOnTexture,[],rect1+[0 0.05*pms.wRect(4) 0 0.05*pms.wRect(4)]);        

        case 4
            message = 'But if you see the inactive boostray with the cross, you can only collect the normal blue treasure.\n\n\n\n\n\n\n';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
...
%            
        ...
        case 5
            message = 'Let''s see how this works. For example, let''s say that you would see the active boostray at the start of the trial:\n\n\n\n\n\n\nThen, if you were to get 3 pieces of space treasure, your total score would go up 15 points.\n\nHowever, if you were to get 9 pieces of treasure, your total score would go up 45 points.\n\nIt is important to know that whether your boost ray works or not on each trial is chosen COMPLETELY RANDOMLY. This means that there is no way to predict what it will be on the next trial.';
            itemsize = 150;
...
if part == 10 % 3 screens
    switch screen
        ...
        case 2
            message = 'Remember:\n\n1. Choose a spaceship to take you to the red or the purple planet. Each spaceship will always take you to the same planet.\n\n2. Press SPACE to ask the aliens for the treasure they have collected.\n\n3. The amount of treasure each alien collects changes over time. Try to think about which alien will give you the most treasure every time.\n\n4. Your boost ray can turn the treaure you collect into gold treasure that is worth 5 TIMES more than the blue treasure!';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
            
            planetsize = 75;
            rect1 = [pms.origin(1)-7*planetsize pms.origin(2)-2.5*planetsize ...
                pms.origin(1)-5.5*planetsize pms.origin(2)-1.5*planetsize];
            Screen('DrawTexture',pms.wid,pms.examplePlanetsSmallTexture,[],rect1+[0 0.05*pms.wRect(4) 0 0.05*pms.wRect(4)]);
            
            itemsize = 75;
            rect2 = [pms.origin(1)+5.75*itemsize pms.origin(2)-1.5*itemsize ...
                pms.origin(1)+6.75*itemsize pms.origin(2)-0.5*itemsize];
            Screen('DrawTexture',pms.wid,pms.BlueTreasureTexture,[],rect2+[0 0.05*pms.wRect(4) 0 0.05*pms.wRect(4)]);
            
            arrowsize = [150 40]*0.75;
            rect3 = [pms.origin(1)-5*arrowsize(1) pms.origin(2)+1*arrowsize(2) ... % arrow
                pms.origin(1)-4*arrowsize(1) pms.origin(2)+2*arrowsize(2)];
            Screen('DrawTexture',pms.wid,pms.arrowTexture,[],rect3+[0 0.03*pms.wRect(4) 0 0.03*pms.wRect(4)]);
            
            boostraysize = 75;
            rect4 = [pms.origin(1)+5.75*boostraysize pms.origin(2)+1.1*boostraysize ...
                pms.origin(1)+6.75*boostraysize pms.origin(2)+2.1*boostraysize];
            Screen('DrawTexture',pms.wid,pms.BoostrayOnTexture,[],rect4+[0 0.05*pms.wRect(4) 0 0.05*pms.wRect(4)]);

        case 3
            message = 'Remember, the space treasure that you collect in this game will count towards your payment at the end.\n\nFor every 100 points, you will get 0.33 GBP. On average, people win about 7.6GBP, and some have won more than 12GBP.\n\n\nMake sure you try and collect as much treasure as you can!\n\nGood luck!';
            DrawFormattedText(pms.wid,message,'center','center', [], wrapat, [], [], 1.5);
